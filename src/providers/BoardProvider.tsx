...
function reducer(state: BoardState, action: Action): BoardState {
  switch (action.type) {
    case "SET_TILES":
      ...
      return {...state};
    case "SET_TILE":
      Debug(`Setting tile at x: ${action.x.toString()}, y: ${action.y.toString()}`);
      // const tiles = state.tiles;
      // tiles[action.y][action.x] = action.tile;
      return {...state};
    case "ADD_PIECE":
      Debug(`Adding piece ${action.piece.uid}`);
      ...
      for (let y = 0; y < action.piece.tiles.length; y++) {
        for (let x = 0; x < action.piece.tiles[y].length; x++) {
          if (y + action.piece.y >= state.board.size.y || x + action.piece.x >= state.board.size.x) {
            Debug(`Piece ${action.piece.uid} out of bounds`);
            Debug(`Tile on x: ${(x + action.piece.x).toString()} y: ${(y + action.piece.y).toString()} is out of bounds`);
            return state;
          }
        }
      }
      if (checkPieceOverlap(state.pieces, action.piece)) {
        Debug(`Piece ${action.piece.uid} overlaps with another piece`);
        return state;
      }
      return {...state, pieces: [...state.pieces, action.piece]};
    case "REMOVE_PIECE":
      Debug(`Removing piece at x: ${action.x.toString()}, y: ${action.y.toString()}`);
    ...
    case "MOVE_PIECE":
      Debug(`Moving piece at x: ${action.x.toString()}, y: ${action.y.toString()} to x: ${action.newX.toString()}, y: ${action.newY.toString()}`);
      // eslint-disable-next-line no-case-declarations
      const piece = state.pieces.find(piece => piece.x === state.hoveredPiece?.x && piece.y === state.hoveredPiece.y);
      if (!piece) {
        Debug(`Piece not found at x: ${action.x.toString()}, y: ${action.y.toString()}`);
        return state;
      }
      for (let y = 0; y < piece.tiles.length; y++) {
        for (let x = 0; x < piece.tiles[y].length; x++) {
          if (y + action.newY >= state.board.size.y || x + action.newX >= state.board.size.x) {
            Debug(`Piece ${piece.uid} out of bounds`);
            Debug(`x: ${x.toString()}, y: ${y.toString()}, newX: ${action.newX.toString()}, newY: ${action.newY.toString()}`);
            return state;
          }
        }
      }
      ...
      if (checkPieceOverlap(state.pieces.filter(x => x.uid !== piece.uid), newPiece)) {
        Debug(`Piece ${piece.uid} overlaps with another piece`);
        return state;
      }
      return {
        ...state,
    ...
    case "HOVER_PIECE":
      ...
      Debug(`Hovering piece ${action.piece.uid}`);
      return {...state, hoveredPiece: action.piece};
    case "LOAD_LEVEL":
      Debug("Loading level", action.level);
      return {
...
function checkPieceOverlap(pieces: BoardPiece[], newPiece: BoardPiece): boolean {
  ...
  Debug("Checking overlap for piece ", newPiece.uid);
  Debug("Positions of other pieces", positions);
  for (let y = 0; y < newPiece.tiles.length; y++) {
    for (let x = 0; x < newPiece.tiles[y].length; x++) {
      Debug(`Checking x: ${(x + newPiece.x).toString()} y: ${(y + newPiece.y).toString()} against positions`, positions.some(([px, py]) => px === newPiece.x + x && py === newPiece.y + y));
